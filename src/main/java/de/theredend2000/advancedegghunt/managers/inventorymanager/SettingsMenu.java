...
public class SettingsMenu extends InventoryMenu {
    ...
    private void menuContent() {
        ...
        getInventory().setItem(22, new ItemBuilder(XMaterial.CLOCK).setDisplayname("§3Hint cooldown on fail").setLore("§7Set if the cooldown of the hint function", "§a applies if the player fails.", "", Main.getInstance().getPluginConfig().getHintApplyCooldownOnFail() ? "§a§l✔ Enabled" : "§c§l❌ Disabled", "§eClick to toggle.").setLocalizedName("settings.hintcooldown").withGlow(Main.getInstance().getPluginConfig().getHintApplyCooldownOnFail()).build());
    }
