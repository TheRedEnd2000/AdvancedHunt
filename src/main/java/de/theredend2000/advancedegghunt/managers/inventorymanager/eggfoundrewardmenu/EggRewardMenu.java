        ...
        if(keys != null && !keys.isEmpty()) {
            for(int i = 0; i < getMaxItemsPerPage(); i++) {
                ...
                if (keys.get(index) != null){
                    ...
                    inventory.addItem(new ItemBuilder(Main.getInstance().getPluginConfig().getRewardInventoryMaterial()).setDisplayname("§b§lCommand §7#" + keys.get(index)).setLore("", "§9Information:", "§7Command: §6" + command, "§7Command Enabled: " + (enabled ? "§atrue" : "§cfalse"), "§7Type: §6" + type, "", "§a§lNote:", "§2Type 0:", "§7Type 0 means that this command will be", "§7be executed if the player found §7§lone §7egg.", "§2Type 1:", "§7Type 1 means that this command will be", "§7be executed if the player had found §7§lall §7egg.", "", "§eClick to configure the command.").setLocalizedName(keys.get(index)).build());
                }
            }
        }else{
            inventory.setItem(22, new ItemBuilder(XMaterial.RED_STAINED_GLASS).setDisplayname("§4§lNo Commands").setLore("§7You can add commands by using", "§e/egghunt placeEggs§7.").build());
