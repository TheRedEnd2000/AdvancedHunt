...
public class InventoryManager {
    ...
    public void createCommandSettingsMenu(Player player, String key) {
        ...
        inventory.setItem(22, new ItemBuilder(Main.getInstance().getMaterial(Main.getInstance().getPluginConfig().getRewardInventoryMaterial())).setDisplayname("§b§lCommand §7#" + key).setLore("", "§9Information:", "§7Command: §6" + command, "§7Command Enabled: " + (enabled ? "§atrue" : "§cfalse"), "§7Type: §6" + type, "", "§a§lNote:", "§2Type 0:", "§7Type 0 means that this command will be", "§7be executed if the player found §7§lone §7egg.", "§2Type 1:", "§7Type 1 means that this command will be", "§7be executed if the player had found §7§lall §7egg.").setLocalizedName(key).build());
        inventory.setItem(29, new ItemBuilder(type == 0 ? XMaterial.WATER_BUCKET : XMaterial.LAVA_BUCKET).setDisplayname("§3Command Type").setLore("§7Change if the command will be executed", "§7if the player founds one or all eggs.", "", "§5Currently:", "§6§l" + type + " §7§l(" + (type == 0 ? "One egg found" : "All eggs found") + ")", "§8More information at the main information. (" + Main.getInstance().getPluginConfig().getRewardInventoryMaterial().toUpperCase() + ")", "", "§eClick to toggle.").setLocalizedName("command.type").build());
