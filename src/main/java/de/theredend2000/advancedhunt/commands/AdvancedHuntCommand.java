            ...
            case 2:
                if (args[0].equalsIgnoreCase("reset") && plugin.getPermissionManager().checkPermission(sender, Permission.Command.RESET)) {
                    ...
                    for (UUID uuid : Main.getInstance().getEggDataManager().savedPlayers()) {
                        String playerData = Main.getInstance().getPlayerEggDataManager().getPlayerData(uuid).getString("Found.");
                        if (playerData == null) {
                            return completions;
                        }
                        for (String eggId : Main.getInstance().getPlayerEggDataManager().getPlayerData(uuid).getConfigurationSection("Found.").getKeys(false)) {
                            String playerName = Main.getInstance().getPlayerEggDataManager().getPlayerData(uuid).getString("Found." + eggId + ".Name");
