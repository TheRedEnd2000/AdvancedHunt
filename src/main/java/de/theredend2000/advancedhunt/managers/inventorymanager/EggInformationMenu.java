...
public class EggInformationMenu extends PaginatedInventoryMenu implements IInventoryMenuOpen {
    ...
    public void setMenuItems(String eggId) {
        ...
        for(UUID uuids : Main.getInstance().getEggDataManager().savedPlayers()){
            ...
            if(playerConfig.contains("Found." + collection + "." + eggId)){
                Collections.addAll(keys, playerConfig.getString("Found." + collection + ".Name"));
        ...
        if(keys != null && !keys.isEmpty()) {
            for(int i = 0; i < getMaxItemsPerPage(); i++) {
                ...
                if (keys.get(index) != null){
                    ...
                    String eggsFound = Main.getInstance().getPlayerEggDataManager().getPlayerData(UUID.fromString(uuid.get(index))).getString("Found." + collection + ".Count");
    ...
    public void handleMenu(InventoryClickEvent event) {
        ...
        String collection = Main.getInstance().getEggManager().getEggCollectionFromPlayerData(playerMenuUtility.getOwner().getUniqueId());
        for(UUID uuids : Main.getInstance().getEggDataManager().savedPlayers()){
            if(Main.getInstance().getPlayerEggDataManager().getPlayerData(uuids).contains("Found." + id)){
                Collections.addAll(keys, Main.getInstance().getPlayerEggDataManager().getPlayerData(uuids).getString("Found." + collection + ".Name"));
            }
        }
    ...
    public int getMaxPages(String eggId){
        ...
        for(UUID uuids : Main.getInstance().getEggDataManager().savedPlayers()){
            FileConfiguration playerConfig = Main.getInstance().getPlayerEggDataManager().getPlayerData(uuids);
            if(playerConfig.contains("Found." + eggId)){
                Collections.addAll(keys, playerConfig.getString("Found.Name"));
            }
        }
