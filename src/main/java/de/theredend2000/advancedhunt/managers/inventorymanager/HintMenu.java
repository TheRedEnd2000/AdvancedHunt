...
public class HintMenu extends InventoryMenu {
    ...
    public void open() {
        if (Main.getInstance().getPluginConfig().getHintCount() < 1) {
            Success();
            return;
        }
    ...
    private void restartFailedTask(){
        ...
        }.runTaskLater(Main.getInstance(), Main.getInstance().getPluginConfig().getHintUpdateTime() + 5);
    }
    ...
    private void UpdateFrame(boolean timeout) {
        ...
        getInventory().setItem(currentSlot, new ItemBuilder(XMaterial.RED_STAINED_GLASS_PANE)
                .setDisplayName("§c")
                .build());
        currentSlot = getRandomSlot();
        getInventory().setItem(currentSlot, new ItemBuilder(XMaterial.LIME_STAINED_GLASS_PANE)
                .setDisplayName("§aConfirm")
                .setLore("§6" + (currentCount + 1) + "§7/§6" + Main.getInstance().getPluginConfig().getHintCount())
                .build());
