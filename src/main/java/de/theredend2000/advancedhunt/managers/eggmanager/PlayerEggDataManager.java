        ...
        new BukkitRunnable() {
            ...
            public void run() {
                for(UUID uuid : plugin.getEggDataManager().savedPlayers()){
                    ...
                    if(!cfg.contains("Found.")) continue;
                    for(String collection : cfg.getConfigurationSection("Found.").getKeys(false)) {
                        for(String eggId : cfg.getConfigurationSection("Found." + collection).getKeys(false)) {
