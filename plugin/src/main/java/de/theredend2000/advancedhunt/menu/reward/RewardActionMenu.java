...
public class RewardActionMenu extends Menu {
    ...
    private void editChance() {
        ...
        plugin.getChatInputListener().requestInput(playerMenuUtility, chanceStr -> {
            try {
                ...
                playerMenuUtility.sendMessage(plugin.getMessageManager().getMessage("feedback.rewards.updated", "%type%", plugin.getMessageManager().getMessage("common.chance", false)));
                parentMenu.open();
    ...
    private void editMessage(InventoryClickEvent e) {
        ...
        if(clickType == ClickType.LEFT || clickType == ClickType.SHIFT_LEFT){
            ...
            plugin.getChatInputListener().requestInput(playerMenuUtility, message -> {
                ...
                playerMenuUtility.sendMessage(plugin.getMessageManager().getMessage("feedback.rewards.updated", "%type%", plugin.getMessageManager().getMessage("common.message", false)));
                parentMenu.open();
            });
        }else if(clickType == ClickType.RIGHT || clickType == ClickType.SHIFT_RIGHT){
            parentMenu.updateRewardMessage(rewardIndex,null);
            playerMenuUtility.sendMessage(plugin.getMessageManager().getMessage("feedback.rewards.reset", "%type%", plugin.getMessageManager().getMessage("common.message", false)));
            parentMenu.open();
        }
    }
    ...
    private void editBroadcast(InventoryClickEvent e) {
        ...
        if(clickType == ClickType.LEFT || clickType == ClickType.SHIFT_LEFT){
            ...
            plugin.getChatInputListener().requestInput(playerMenuUtility, message -> {
                ...
                playerMenuUtility.sendMessage(plugin.getMessageManager().getMessage("feedback.rewards.updated", "%type%", plugin.getMessageManager().getMessage("common.broadcast", false)));
                parentMenu.open();
            });
        }else if(clickType == ClickType.RIGHT || clickType == ClickType.SHIFT_RIGHT){
            parentMenu.updateRewardBroadcast(rewardIndex,null);
            playerMenuUtility.sendMessage(plugin.getMessageManager().getMessage("feedback.rewards.reset", "%type%", plugin.getMessageManager().getMessage("common.broadcast", false)));
            parentMenu.open();
        }
    }
    ...
    private String formatMessage(String message){
        if(message == null) return plugin.getMessageManager().getMessage("gui.common.none", false);
        return message;
    }
